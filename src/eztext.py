"""
    ...
    Input lib, used in highscores to get player name
...
class Input:
    ...
    def __init__(self, **options):
        ...
        self.shifted = False
        self.finished = False
        self.w, self.h = self.font.size(self.prompt)
        # load cup image
        self.mCupImg = pygame.image.load(os.path.join('..', 'png', 'goldcup.png'))
    ...
    def draw(self, surface):
        ...
        pygame.draw.rect(surface, (200, 200, 200), pygame.Rect(self.x - 10, self.y - 10, self.mCupImg.get_rect().width + self.w + 30, self.mCupImg.get_rect().height + 20), 0)
        pygame.draw.rect(surface, (100, 100, 100), pygame.Rect(self.x, self.y, self.mCupImg.get_rect().width + self.w + 10, self.mCupImg.get_rect().height), 0)
        tools.drawAligned(surface, self.mCupImg, self.x, self.y)
        #text = self.font.render(self.prompt+self.value, 1, self.color)
        prompt = self.font.render(self.prompt, 1, self.color)
        text = self.font.render('   Your name: ' + str(self.value), 1, self.color)
        surface.blit(prompt, (self.mCupImg.get_rect().width + self.x, self.y + 10))
        surface.blit(text, (self.mCupImg.get_rect().width + self.x, self.y + self.mCupImg.get_rect().height // 2))
    ...
    def update(self, events):
        ...
        for event in events:        
            ...
            if event.type == KEYDOWN:
                if event.key == K_RETURN: self.finished = True                
